...
public class MfComplexData: MfDataProtocol{
    ...
    public init(size: Int, mftype: MfType){
        ...
        switch MfType.storedType(mftype){
        case .Float:
            let ptrrF = allocate_unsafeMPtrT(type: Float.self, count: size)
            let ptriF = allocate_unsafeMPtrT(type: Float.self, count: size)
            
            self.data_real = UnsafeMutableRawPointer(ptrrF)
            self.data_imag = UnsafeMutableRawPointer(ptriF)
        case .Double:
            let ptrrD = allocate_unsafeMPtrT(type: Double.self, count: size)
            let ptriD = allocate_unsafeMPtrT(type: Double.self, count: size)
            
            self.data_real = UnsafeMutableRawPointer(ptrrD)
    ...
    /// Create a MfData with VIEW based on base mfdata
    /// - Parameters:
    ///   - refdata: The base mfcomplexdata
    ///   - offset: The offset value from base's data
    public init(refdata: MfComplexData, offset: Int) {
        self._base = refdata
    ...
    public init(ref_realdata: MfData, ref_imagdata: MfData, offset: Int) {
        ...
        memcpy(datarptr, ref_realdata.data + ref_realdata.byteOffset, bytesize)
        memcpy(dataiptr, ref_imagdata.data + ref_imagdata.byteOffset, bytesize)
    ...
    deinit {
        if !self._isView{
            func _deallocate<T: MfStorable>(_ type: T.Type){
                let datarptr = self.data_real.bindMemory(to: T.self, capacity: self.storedSize)
                datarptr.deinitialize(count: self.storedSize)
                datarptr.deallocate()
                
                let dataiptr = self.data_imag.bindMemory(to: T.self, capacity: self.storedSize)
