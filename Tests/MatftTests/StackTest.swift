...
final class StackTests: XCTestCase {
    ...
    func test_concatenate(){
        ...
        do{
            let a = Matft.mfarray.arange(start: 0, to: 45, by: 1, shape: [15,3])
            let b = Matft.mfarray.arange(start: 0, to: 45, by: 1, shape: [15,3])
            
            let P = Matft.mfarray.concatenate([a[0~-1], b[1~]], axis: 0).T
